...
function detectEngine(): string {
	const hermes = (globalThis as { HermesInternal?: { getRuntimeProperties?: () => Record<string, string> } }).HermesInternal;
...
export const GLOBALS: DemoGlobalsReport = {
	...setupGrabitGlobals({ crypto: QuickCrypto, buffer: Buffer, env: { WYZIE_SUBS_KEYS: process.env.EXPO_PUBLIC_WYZIE_SUBS_KEYS } }),
	engine: detectEngine()
};
