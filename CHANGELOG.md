...
- Work continuing after an operation timed out or was cancelled. Three gaps: (1) the per-provider retry (`excuteWithRetries`) was never given the operation's abort signal, so a failing provider kept re-issuing attempts ("Attempt N failed. Retrying…") after the manager gave up; (2) `ctx.solveChallenge` ignored `requester.signal`, so a provider's challenge loop kept opening new challenges — on native this runs through the host-injected solver (RN WebView / FlareSolverr), each with its own ~20s timeout, so a scrape could run tens of seconds past the operation timeout; (3) the Node puppeteer solver's cookie-wait loop also ignored the signal. All three now honor the abort: retries stop, and `solveChallenge` rejects immediately when the operation is (or becomes) aborted so providers stop chaining more challenge work. Note: a host solver should also watch `requester.signal` to tear down its in-flight WebView; the engine can only stop *new* work from being issued. Together these make timeouts and `closeOperations()` (the RN hook's stop/unmount) actually halt in-flight work.
- Native crash "undefined is not a function" when an operation cleared its queue (timeout, quorum, or `closeOperations`). The per-operation limiter uses p-limit's `rejectOnClear`, whose `clearQueue()` calls the static `AbortSignal.abort()` — which Hermes/React Native does not implement. A guarded spec-equivalent shim is now installed when that static is missing, matching how `AbortSignal.any` is already worked around.
