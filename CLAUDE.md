...
- `provider.ts` — `defineProviderModule()` wraps a provider's `getStreams`/`getSubtitles` to inject metadata (fileName, providerName, scheme, User-Agent header), sort by target language, optionally validate source URLs, and normalize error logging. Provider modules pass through here whether loaded from GitHub, local, or registry. It also wires `resolveLazy` for **lazy sources**: `MediaSource = ResolvedMediaSource | LazyMediaSource`, where a lazy source returns `{ lazy: { id } }` with no `playlist` and is resolved on play via `manager.resolveLazySource(scheme, id, requester)`. Discriminate with `source.lazy`; validation skips lazy sources. Playback hints are `xhr.flags: SourceFlag[]` (replaced the old `haveCorsPolicy` boolean).
